...
describe('Utility Functions', () => {
  ...
  describe('Category Configuration', () => {
    ...
    it('uses Arabic labelAr for database API poet parameter', () => {
      ...
      expect(buildPoetParam(CATEGORIES, 'Mahmoud Darwish')).toBe(
        `?poet=${encodeURIComponent('محمود درويش')}`
      );
      expect(buildPoetParam(CATEGORIES, 'Nizar Qabbani')).toBe(
        `?poet=${encodeURIComponent('نزار قباني')}`
      );
      // Fallback: unknown category uses the id as-is
      expect(buildPoetParam(CATEGORIES, 'Unknown Poet')).toBe(
        `?poet=${encodeURIComponent('Unknown Poet')}`
      );
    });
