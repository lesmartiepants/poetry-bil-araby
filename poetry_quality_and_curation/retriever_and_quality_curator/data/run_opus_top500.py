"""Opus re-score: top 500 poems for 90+ tier identification.

Usage:
    python -m poetry_quality_and_curation.retriever_and_quality_curator.data.run_opus_top500

Model: Opus (best quality, best spread from experiments)
Input: top 500 from scores_recalibrated_v5.parquet
Output: data/scores_opus_top500.parquet
Budget: ~$10
"""
import asyncio
import json
...
# We also need the poem content from final_selection_v4
CONTENT_PATH = DATA_DIR / "final_selection_v4.parquet"
OUTPUT_PATH = DATA_DIR / "scores_opus_top500_v2.parquet"

MODEL = "openai/bedrock-opus-46"
N_TOP = 500
BATCH_SIZE = 3  # Opus is slower, smaller batches
CONCURRENCY = 5  # Lower concurrency for Opus
MAX_COST = 12.0
CHECKPOINT_INTERVAL = 100

DIMS = ["sound", "imagery", "emotion", "language", "cultural"]

# Opus-specific prompt: variant A base + refined bands for full 0-100 use
OPUS_PROMPT = f"""أنت ناقد أدبي عربي متخصص في الشعر العربي الكلاسيكي والحديث. مهمتك تقييم القصائد بدقة على مقياس مُعايَر بأمثلة مرجعية حقيقية، باستخدام النطاق الكامل من 0 إلى 100.
...
- **0-30**: ليس شعراً — كلمات عشوائية أو نص مكسور أو مفكك
- **30-50**: باهت بلا صنعة — نظم منثور، سرد تاريخي مقفّى، مدح عام بلا تصوير، أبيات مباشرة بلا مجاز
- **50-75**: شعر كفء لكنه عادي — وزن سليم وقافية مقبولة لكن لا شيء يجعلك تتوقف وتعيد القراءة. هذا سقف المهارة التقنية بدون تميز.
...
**مفتاح التوزيع**: هذه قصائد مختارة من أعلى المجموعة، لذا معظمها جيد. من كل 5 قصائد، توقع أن تجد 1-2 تستحق 90+، و1-2 في 75-89، و1 في 50-74.

**سقف "الكفاءة" = 75**: القصيدة التي فيها وزن سليم وقافية مقبولة لكن بلا صور ولا موسيقى داخلية ولا مفاجآت لغوية = لا تتجاوز 75.

**علامات الشعر الباهت (أقل من 50):**
- القصائد الطويلة التي تشبه النثر المسجوع أو السرد التاريخي المنظوم — تأريخ منظوم لا شعر
- المدح العام المكرر بلا ابتكار — مجرد إطراء مقفّى
- الأبيات القصيرة الخالية من الصور والموسيقى — لا مجاز، لا استعارة، لا جرس موسيقي
- القِصَر ليس ميزة بذاته — قصيدة قصيرة جميلة تختلف عن قصيدة قصيرة باهتة

**علامات الشعر العالي (90+) — كن سخياً مع الشعر الجيد:**
- الشعر الحقيقي يُسمع قبل أن يُقرأ — الموسيقى تسري في كل بيت
- الصور الشعرية تبقى في الذاكرة بعد القراءة
- العاطفة صادقة تُحسّ — القصيدة تجعلك تشعر بشيء
- اللغة فيها مفاجآت — ألفاظ غير متوقعة وتراكيب جميلة
...
async def score_batch(batch_df: pd.DataFrame, semaphore: asyncio.Semaphore) -> tuple[list[dict], float]:
    ...
    async with semaphore:
        ...
        kwargs = dict(
            model=MODEL,
            messages=[
                {"role": "system", "content": OPUS_PROMPT},
...
async def main():
    print("=" * 60)
    print("OPUS RE-SCORE: Top 500 poems for 90+ tier identification")
    print("=" * 60)

    # Load haiku-calibrated scores to find top 500
    scores_df = pd.read_parquet(INPUT_PATH)
    top_ids = set(scores_df.nlargest(N_TOP, "quality_score")["poem_id"].astype(str).tolist())

    # Load content from final_selection_v4
    content_df = pd.read_parquet(CONTENT_PATH)
    poems_df = content_df[content_df["poem_id"].astype(str).isin(top_ids)].copy()
    print(f"Loaded {len(poems_df)} top poems (by haiku-calibrated score)")
    print(f"  Score range: {scores_df[scores_df['poem_id'].isin(top_ids)]['quality_score'].min()}-{scores_df[scores_df['poem_id'].isin(top_ids)]['quality_score'].max()}")
    ...
    all_scores = []
    total_cost = 0.0
    pbar = tqdm(total=len(poems_df), desc="Opus scoring")
    ...
    if OUTPUT_PATH.exists():
        ...
        print(f"\n{'='*60}")
        print(f"OPUS RE-SCORE COMPLETE")
        print(f"{'='*60}")
