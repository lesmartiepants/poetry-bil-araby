...
describe('highlightStyle in uiStore', () => {
  it('initialises with highlightStyle "none"', () => {
    expect(useUIStore.getState().highlightStyle).toBe('none');
  });
  ...
  it('reset returns highlightStyle to "none"', () => {
...
describe('TextSettingsPill — Row 5 Highlight selector', () => {
  ...
  it('"Off" button is active by default (highlightStyle is "none")', () => {
    render(<TextSettingsPill />);
    fireEvent.click(screen.getByRole('button', { name: /text settings/i }));
    const offBtn = screen.getByRole('radio', { name: /off/i });
    expect(offBtn).toHaveAttribute('data-state', 'on');
  });
