 ...
 *
 * These tests run against a REAL PostgreSQL database (local service in CI,
 * or a local dev instance). They are excluded from the normal `npm test` run.
 *
 * Run via: npm run test:integration
 *
 * Required: Either DATABASE_URL env var or individual PG* env vars
 * (PGUSER, PGPASSWORD, PGDATABASE, PGHOST, PGPORT).
 *
 * In CI, the PostgreSQL service container in ci.yml provides these.
 * Test data uses reviewer='ci-integration' for easy identification.
 */
...
// Skip when no database is configured. In CI the e2e-tests job provides PG env vars.
const hasDatabase = !!(process.env.DATABASE_URL || process.env.PGDATABASE);

describe.skipIf(!hasDatabase)('Design Review — Real Database Integration', () => {
  let app;
  let pool;
  let sessionId;
  ...
  beforeAll(async () => {
    ...
    // Pre-flight: verify DB connectivity and log available tables.
    ...
    if (missing.length > 0) {
      throw new Error(
        ...
        `Run the migration (20260222000000_design_review_tables.sql) and retry.`
      );
    }
  });
