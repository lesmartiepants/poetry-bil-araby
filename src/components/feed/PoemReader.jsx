...
const PoemReader = memo(function PoemReader({
  ...
  const { revealedCount, isAllRevealed, advance, revealAll } = useStanzaReveal(
    stanzas,
    poem?.id
  );
  ...
  return (
    <div
      className="flex flex-col items-center w-full px-4 md:px-20 py-2 text-center"
        ...
        {stanzas.length > 1 && !useHighlight && (
          <div className="flex justify-center gap-2 mt-6" dir="ltr" aria-label="Stanza progress">
            {stanzas.map((_, i) => (
              <div
                key={i}
                aria-label={i < revealedCount ? `Stanza ${i + 1} revealed` : `Stanza ${i + 1} hidden`}
                style={{
