...
| `reader-feed` | critical | mocked | - | #714 N4 port. e2e drives the column with mocked poems. Not behaviorally asserted: the 760ms hold and its cancel decay, over-pull charging, TTS follow inside the scroller, and the reduced-motion path where the dissolve animation never fires. iOS long-press suppression and the 90-mote arrival cost are device-only. |
