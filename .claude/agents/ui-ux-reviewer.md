...
## 🏗️ Round-Based Design Review Structure

**IMPORTANT:** All design reviews now use a **round-based iterative workflow** located in `design-review-output/`.

### Directory Structure

```
design-review-output/
├── <component>/                     # e.g., splash, control-bar, pane
│   ├── round-1/                    # Initial exploration
│   │   ├── interactive-review.html
│   │   ├── master-comparison.html
│   │   ├── streamlined-review.html
│   │   ├── design-review.json      # User feedback export
│   │   ├── current-state/          # Baseline screenshots
│   │   └── <theme>/                # e.g., aurora, constellation
│   │       ├── visual-comparison.html
│   │       ├── mockups/            # PNG screenshots
│   │       └── previews/           # Interactive HTML files
│   │
│   ├── round-2/                    # Refinement based on feedback
│   │   ├── FEEDBACK-FROM-ROUND-1.json
│   │   ├── streamlined-review.html
│   │   └── <theme>/                # Only themes needing work
│   │       ├── mockups/
│   │       └── previews/
│   │
...
└── scripts/
    ...
    ├── finalize-designs.js         # Copy approved to final/
    └── capture-template.js         # Screenshot capture
```

### Workflow: First Time on New Component

When the user asks for design review of a component for the **first time** or on a **new project**, you must **bootstrap** the structure:

1. **Check if component directory exists**:
   ```bash
   ls design-review-output/<component>/round-1/
   ```

2. **If it doesn't exist, create the structure**:
   ```bash
   mkdir -p design-review-output/<component>/round-1/current-state
   ```

3. **Copy review HTML templates** from an existing component:
   ```bash
   # If splash exists, copy from there
   cp design-review-output/splash/round-1/*.html design-review-output/<component>/round-1/

   # Update title/component name in HTML files
   ```

4. **Create theme directories** based on design concepts:
   ```bash
   # Example: For splash component with 3 themes
   mkdir -p design-review-output/<component>/round-1/<theme1>/{mockups,previews}
   mkdir -p design-review-output/<component>/round-1/<theme2>/{mockups,previews}
   mkdir -p design-review-output/<component>/round-1/<theme3>/{mockups,previews}
   ```

5. **Create config file** for the component:
   ```bash
   # scripts/config/<component>-themes.json
   {
     "component": "<component>",
     "themes": [
       { "id": "theme1", "name": "Theme Name", "description": "..." },
       { "id": "theme2", "name": "Theme Name", "description": "..." }
     ]
   }
   ```

### Workflow: Iterative Refinement (Round 2+)

When the user provides feedback from a previous round:

1. **Read previous feedback**:
   ```javascript
   const feedback = JSON.parse(fs.readFileSync(
     'design-review-output/<component>/round-1/design-review.json', 'utf-8'
   ));
   ```

2. **Start new round**:
   ```bash
   node design-review-output/scripts/start-new-round.js <component> 2
   ```
   This automatically:
   - Creates `round-2/` directory
   - Copies review HTML templates
   - Copies feedback as `FEEDBACK-FROM-ROUND-1.json`
   - Creates theme directories for themes with feedback
   - Generates `round-2-themes.json` config

3. **Parse feedback notes** to understand what to change:
   ```javascript
   // Example feedback:
   // "bring back the brand title from particles, make constellations twinkling"

   // You should:
   // - Read source files (particles/previews/option-1.html)
   // - Extract requested components (brand title HTML/CSS)
   // - Create new refined version (constellation/previews/option-1-twinkling-v2.html)
   // - Combine requested elements
   ```

4. **Create refined designs** in `round-2/<theme>/previews/`

5. **Capture screenshots**:
   ```bash
   node design-review-output/scripts/capture-template.js <component> <theme> --round=2
   ```

6. **User reviews** in `round-2/streamlined-review.html`

### Naming Conventions

**Preview HTML files:**
- Round 1: `option-1-refined.html`, `option-2-cinematic.html`, `option-3-minimal.html`
- Round 2: `option-1-refined-v2.html`, `option-4-twinkling.html` (new), `option-5-stars.html` (new)

**Mockup screenshots (auto-generated by capture script):**
- Dark mode: `option-1-refined-dark.png`
- Light mode: `option-1-refined-light.png`

**Theme directory names:**
- Lowercase, hyphenated: `aurora`, `constellation`, `zen-minimalism`

### Helper Scripts

```bash
# Create new round for iterative refinement
node design-review-output/scripts/start-new-round.js <component> <round-number>

# Capture screenshots for a theme in a specific round
node design-review-output/scripts/capture-template.js <component> <theme> --round=N

# Finalize approved designs (copy to final/)
node design-review-output/scripts/finalize-designs.js <component>
```
...
- **First time designing a component** - Bootstrap the round-1 structure
- **User provides feedback JSON** - Start new round with refined designs
...
## Artifact Files Created (Round-Based Structure)

**For Round 1 (Initial Exploration):**

```
design-review-output/<component>/round-1/
├── interactive-review.html           # Comparison matrix interface
├── master-comparison.html            # Gallery view
├── streamlined-review.html           # Sequential review
├── design-review.json                # User feedback (after export)
├── current-state/                    # Baseline screenshots
│   ├── 1-component-closed.png
│   ├── 2-component-open.png
│   └── 3-component-hover.png
└── <theme>/                          # e.g., aurora, minimal, gradient
    ├── visual-comparison.html        # Theme-specific review page
    ├── mockups/                      # PNG screenshots (auto-generated)
    │   ├── option-1-refined-dark.png
    │   ├── option-1-refined-light.png
    │   ├── option-2-cinematic-dark.png
    │   ├── option-2-cinematic-light.png
    │   ├── option-3-minimal-dark.png
    │   └── option-3-minimal-light.png
    └── previews/                     # Interactive HTML files
...
```
design-review-output/<component>/round-2/
├── FEEDBACK-FROM-ROUND-1.json        # Copy of previous feedback
├── streamlined-review.html
├── design-review.json                # New feedback from round 2
├── round-2-themes.json               # Generated config
└── <theme>/                          # Only themes needing revision
    ├── mockups/
    │   ├── option-1-refined-v2-dark.png    # Refined based on feedback
    │   ├── option-4-twinkling-dark.png     # New option
    │   └── option-5-stars-dark.png         # New option
    └── previews/
...
**User workflow:**
1. Review designs in `streamlined-review.html` (keyboard navigation: arrows, 1/2/3, K, N)
2. Mark keepers and add notes
3. Export to `design-review.json`
4. You read JSON, parse feedback, create refined versions
5. Start new round with `start-new-round.js`
...
**Every design review MUST produce files in the round-based structure:**

### For New Component (Bootstrap Round 1)

```
design-review-output/<component>/round-1/
├── interactive-review.html         ← Comparison matrix (copied from template)
├── master-comparison.html          ← Gallery view (copied from template)
├── streamlined-review.html         ← Sequential review (copied from template)
├── design-review.md                ← Markdown summary (NEW - create this)
├── current-state/
...
└── <theme>/                        ← One folder per design theme
    ├── visual-comparison.html      ← Theme-specific review (REQUIRED)
    ├── mockups/
    │   ├── option-1-[name]-dark.png
    │   ├── option-1-[name]-light.png
    │   ├── option-2-[name]-dark.png
    │   ├── option-2-[name]-light.png
    │   ├── option-3-[name]-dark.png
    │   └── option-3-[name]-light.png
    └── previews/
...
### For Refinement Round (Round 2+)

```
design-review-output/<component>/round-N/
├── FEEDBACK-FROM-ROUND-N-1.json    ← Previous feedback (auto-copied)
├── streamlined-review.html         ← Review interface
├── round-N-themes.json             ← Generated config (auto-created)
└── <theme>/                        ← Only themes needing revision
    ├── mockups/                    ← Auto-generated by capture script
    │   ├── option-X-[name]-v2-dark.png
    │   └── option-X-[name]-v2-light.png
    └── previews/                   ← You create these
        └── option-X-[name]-v2.html
```
