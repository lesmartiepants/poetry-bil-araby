 ...
 *
 * Desktop: Floats as a narrow card beside the poem — no scrim,
 *   no full-screen takeover. The main poem stays readable.
 *
 * Mobile: Bottom sheet (70% height) — the core Waraqa principle of keeping
 *   the poem partially visible above still works with the sheet open.
 */
const SavedPoemsView_E_Waraqa = ({
  ...
  // Compute once on mount — avoids re-render on resize for this tester component
  ...
  const glass = darkMode ? 'rgba(19,17,14,0.95)' : 'rgba(255,252,244,0.97)';
  ...
  // Shared inner content (same for both mobile sheet and desktop card)
  const innerContent = (
    <>
      {/* Drag handle */}
            ...
            {group.items.map((poem) => {
              ...
              return (
                <div
                  ...
                  {/* Hover / always-visible-on-mobile actions */}
                  <div style={{ display: 'flex', gap: 3, flexShrink: 0, opacity: isHov || isMobile ? 1 : 0, transition: 'opacity 120ms', paddingTop: 2 }}>
                    <button
                    ...
                    <button
                      ...
                      style={{
                        width: 28, height: 28, borderRadius: 6,
                        border: darkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.08)',
                        background: pinnedIds.has(poem.id) ? 'rgba(197,160,89,0.12)' : 'transparent',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                      }}>
                      <Pin size={12} color={pinnedIds.has(poem.id) ? gold : txtMute} />
                    </button>
                    <button
      ...
      <div style={{
        ...
        <button
          onClick={async () => {
            const text = savedPoems
              .map((p) => `${p.title || '—'}\n${p.poet || ''}\n\n${(p.poem_text || '').split('\n').slice(0, 4).join('\n')}`)
              .join('\n\n---\n\n');
            try {
              if (navigator.share) await navigator.share({ title: 'قصائدي المحفوظة', text });
  ...
  if (isMobile) {
    return (
      <AnimatePresence>
        {/* Scrim — clicking above the sheet closes it */}
        <motion.div
          key="waraqa-scrim"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
  ...
  // Desktop: floating card beside the poem
  return (
    <AnimatePresence>
      <motion.div
        key="waraqa-card"
