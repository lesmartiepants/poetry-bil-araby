...
describe('Design Review API', () => {
  ...
  describe('Full persist-to-DB flow (Send to Backend)', () => {
    it('should return 503 when tables are missing during session creation', async () => {
      ...
      mockTablesNotExist(); // for designTablesExist in GET /items
      ...
      // The client must treat this 503 as a fatal error and set apiAvailable=false.
      // Previously initAPI() silently swallowed !cr.ok, leaving currentSession=null
      // but apiAvailable=true — causing confusing "Synced" status.
    });
    ...
    it('should succeed end-to-end when tables exist: create session → save verdicts → complete', async () => {
      ...
      mockTablesExist();
      mockPool.query.mockResolvedValueOnce({ rows: [{ next_round: 1 }] }); // round calc
      mockPool.query.mockResolvedValueOnce({
