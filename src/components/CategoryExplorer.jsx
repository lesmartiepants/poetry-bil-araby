...
export default function CategoryExplorer({
  ...
  useEffect(() => {
    ...
    fetchPoemsByCategory({ ids })
      .then((list) => {
        if (!cancelled) setSavedResults(list);
      })
      .catch(() => {
  ...
  const savedFacets = useMemo(() => {
    ...
    for (const f of data.families)
      famMembers[f.key] = new Set((f.values || []).map((v) => `${v.dim}:${v.key}`));
    for (const p of savedResults) {
      const c = p.categories || {};
      const keys = [];
      (c.moods || []).forEach((k) => {
        valueCount.mood[k] = (valueCount.mood[k] || 0) + 1;
        keys.push(`mood:${k}`);
      });
      (c.topics || []).forEach((k) => {
        valueCount.topic[k] = (valueCount.topic[k] || 0) + 1;
        keys.push(`topic:${k}`);
      });
      (c.motifs || []).forEach((k) => {
        valueCount.motif[k] = (valueCount.motif[k] || 0) + 1;
        keys.push(`motif:${k}`);
      });
      for (const f of data.families) {
        if (keys.some((k) => famMembers[f.key].has(k)))
          familyCount[f.key] = (familyCount[f.key] || 0) + 1;
      }
  ...
  const matchesSavedFilters = (poem) => {
    ...
    for (const dim of data.dimensions) {
      ...
      const ok =
        dimMode[dim.key] === 'and' ? sel.every((v) => have.has(v)) : sel.some((v) => have.has(v));
      if (!ok) return false;
    }
          ...
          {loading && !loaded ? (
            ...
            <div className="flex flex-col gap-3 pt-1">
              ...
              <FilterBar
                data={
                  showSaved
                    ? { dimensions: savedFacets.dimensions, families: savedFacets.families }
                    : data
                }
              ...
              {showSaved && !user ? (
                <div
                  className="px-4 py-12 text-center text-[0.8125rem] font-brand-en"
...
function EmptyState({ subTextColor }) {
  return (
    <div className="px-4 py-16 text-center space-y-2">
      ...
      <span
        className="block text-[0.75rem] font-brand-en"
        style={{ opacity: 0.55, color: subTextColor }}
      >
  ...
  return (
    <div className="flex flex-col gap-5 pt-1">
      ...
      <div className="flex flex-col gap-2">
        <p
          className="font-brand-en text-[0.5625rem] uppercase tracking-[0.15em] px-0.5"
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {fam.poem_count > 0 && (
                    <span
                      className="font-brand-en text-[0.625rem] font-bold"
              ...
              {open && (
                <div className="px-3.5 pb-3 flex flex-col gap-2.5">
                  {Object.entries(grouped).map(([dimKey, vals]) => (
                    <div key={dimKey}>
                      <p
                        className="font-brand-en text-[0.5625rem] uppercase tracking-[0.12em] mb-1"
                      ...
                      <div className="flex flex-wrap gap-1">
                        {vals.map((v) => (
                          ...
                          >
                            <span
                              dir="rtl"
                              style={{ fontFamily: "'Reem Kufi', sans-serif", color: textColor }}
                            >
                              {v.label_ar}
                            </span>
        ...
        {data.dimensions.map((dim) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className="font-brand-en text-[0.5625rem] font-bold"
                style={{ color: 'var(--gold)', opacity: 0.75 }}
              >
...
function FilterBar({
  ...
  const centuryOptions = availableCenturies
    ? CENTURIES.filter((c) => availableCenturies.has(String(c)))
    : CENTURIES;
  ...
  const famLabel = activeFamily
    ...
    : null;
  const poetLabel = poet ? poets.find((p) => p.name === poet)?.name_en || poet : null;
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <FilterButton
          ...
          value={eraLabel}
          open={open === 'era'}
          onClick={() => toggleOpen('era')}
          tokens={tokens}
        />
        <FilterButton
          id="century"
          label="Century"
          value={century ? ordinal(Number(century)) : null}
          open={open === 'century'}
          onClick={() => toggleOpen('century')}
          tokens={tokens}
        />
        <FilterButton
          id="intensity"
          label="Intensity"
          value={intensityActive ? `${intensity[0]}–${intensity[1]}` : null}
          open={open === 'intensity'}
          onClick={() => toggleOpen('intensity')}
          tokens={tokens}
        />
        <FilterButton
          id="difficulty"
          label="Difficulty"
          value={difficultyActive ? `${difficulty[0]}–${difficulty[1]}` : null}
          open={open === 'difficulty'}
          onClick={() => toggleOpen('difficulty')}
          tokens={tokens}
        />
        {activeFilterCount > 0 && (
          <button
      ...
      {data.dimensions.map((dim) =>
        open === dim.key ? (
          <MultiSelectPanel
            ...
            options={[...(dim.values || [])].sort((a, b) =>
              (a.label_en || '').localeCompare(b.label_en || '')
            )}
      ...
      {open === 'intensity' && (
        <div
          ...
          style={{ borderColor: subtleBorder, background: panelBg }}
        >
          <RangeRow
            label="Intensity"
            labelAr="الحِدّة"
...
function FilterButton({ label, value, open, onClick, tokens }) {
  ...
  return (
    ...
    >
      <span>{label}</span>
      {value != null && value !== '' && (
        <span className="truncate opacity-90" style={{ maxWidth: '6rem' }}>
          · {value}
        </span>
      )}
      <ChevronDown
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span
          className="font-brand-en font-bold text-[0.6875rem] uppercase tracking-[0.1em]"
          style={{ color: 'var(--gold)' }}
        >
          {title}
        </span>
            ...
            >
              <span className="flex items-center gap-2 min-w-0">
                <span
                  ...
                  style={{
                    border: `1.5px solid ${on ? 'var(--gold)' : subtleBorder}`,
                ...
                <span
                  dir="ltr"
                  className="font-brand-en font-semibold text-[0.75rem] truncate"
              ...
              <span className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <span
                  ...
                  style={{ fontFamily: "'Reem Kufi', sans-serif", color: subTextColor }}
                >
                  {v.label_ar}
                </span>
              </span>
  ...
  return (
    ...
    >
      ...
      >
            ...
            >
              <span className="flex items-center gap-2 min-w-0">
                ...
                <span
                  dir="ltr"
                  className="font-brand-en font-semibold text-[0.75rem] truncate"
                  style={{ color: on ? 'var(--gold)' : textColor }}
                >
                  {o.en}
                </span>
              </span>
        ...
        {filtered.length === 0 && (
          <span
            className="text-[0.6875rem] font-brand-en px-2 py-2"
            style={{ color: subTextColor }}
          >
            No matches
          </span>
        ...
        {capped && (
          <span
            className="text-[0.625rem] font-brand-en px-2 py-1.5"
            style={{ color: subTextColor, opacity: 0.7 }}
          >
...
function RangeRow({ label, labelAr, value, min, max, step, onChange, subTextColor }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span
          className="font-brand-en font-semibold text-[0.6875rem]"
          style={{ color: subTextColor }}
        >
          {label} ·{' '}
          <span dir="rtl" style={{ fontFamily: "'Tajawal', sans-serif" }}>
            {labelAr}
          </span>
        </span>
