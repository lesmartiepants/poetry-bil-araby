...
import { fetchTags, fetchTagCategories, createTag, updateTag, deleteTag } from '../../services/tags.js';
...
export default function TagAdminPanel({ apiKey, onClose }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [tagsData, catData] = await Promise.all([fetchTags({ limit: 500 }), fetchTagCategories()]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {createError && (
            <div style={{ fontSize: '0.7rem', color: '#f87171', fontFamily: "'Tajawal', sans-serif" }}>
              {createError}
            </div>
          )}
        </div>
                  ...
                  {isEditing ? (
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          setEditValues((p) => ({ ...p, name_ar: e.target.value }))
                        }
                      />
                      <input
                        style={{ ...inputStyle, flex: 1 }}
                        ...
                        onChange={(e) =>
                          setEditValues((p) => ({ ...p, name_en: e.target.value }))
                        }
                      />
                      <button
