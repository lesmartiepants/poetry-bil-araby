...
import { Search, X, SlidersHorizontal, Shuffle, Tag, ChevronDown, ArrowRight, Loader2 } from 'lucide-react';
      ...
      try {
        ...
        if (hasQuery) {
          ...
          for (const result of batch) {
            if (result.status === 'fulfilled' && result.value?.id && !seenIds.has(result.value.id)) {
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted, marginLeft: '0.25rem' }}
              ...
              >
                {SORT_OPTIONS.map((opt) => (
                  ...
                  >
                    ...
                    <span dir="rtl" style={{ fontFamily: "'Amiri', serif", fontSize: '0.7rem', opacity: 0.6 }}>
                      {opt.labelAr}
                    </span>
                  </button>
                ))}
        ...
        <div style={{ overflowY: 'auto', maxHeight: '420px' }}>
          ...
          {!hasActiveFilters && !loading && (
            ...
            >
              ...
              <div dir="rtl" style={{ fontFamily: "'Amiri', serif", opacity: 0.5, fontSize: '0.85rem' }}>
              ...
              {Array.isArray(poem.tags) && poem.tags.length > 0 && (
                <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', marginTop: '0.25rem' }}>
                  {poem.tags.slice(0, 4).map((tag) => {
                    const tagObj = typeof tag === 'object' ? tag : { id: tag, name_ar: tag, name_en: tag };
                    return (
                      <TagBadge
                        key={tagObj.id || tag}
                        tag={tagObj}
