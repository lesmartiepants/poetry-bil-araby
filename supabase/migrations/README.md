...
For a hosted Supabase project, `supabase db push` does the same thing.

Either way you get the **schema only**. There is no poem data in this repo — see
"Where the poems live" below.
...
The 84k-row import (`*_import_poetry.sql`, ~118MB) is gitignored on purpose and
...
The lookup tables also come up empty. `poems` and `poets` have FKs into them, so
loading poem data means loading the reference rows first — both are in the import.
